 ...
 */
// @formatter:on
package eu.matejkormuth.pexel.slave.kits;
...
public class KitInventoryMenu {
    ...
    public KitInventoryMenu(final Kit... kits) {
        ...
        for (int i = 0; i < kits.length; i++) {
            ...
            items.add(new InventoryMenuItem(kit.getIcon(), new JavaArbitraryAction(
                    new ParametrizedRunnable() {
                        @Override
                        public void run(final Object... args) {
                            kit.applyKit(((Player) args[0]));
                            KitInventoryMenu.this.onKitSelected.run(args[0], kit);
                        }
                    }), i, false));
        }
